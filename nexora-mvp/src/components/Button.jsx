...
const variantToClasses = {
  ...
  danger: 'bg-error-dark text-white hover:opacity-90 dark:bg-error-dark light:bg-error-light',
};
...
const Button = ({ variant = 'primary', className = '', children, ...props }) => {
  ...
  return (
    <button
      className={`px-4 py-2 rounded-md font-semibold shadow-gold transition-gold ${classes} ${className}`}
      {...props}
    >
      {children}
    </button>
